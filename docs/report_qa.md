...
During EDA, I found that the dataset has 11 columns and 6 million+ bank transactions.
After doing univariate, bivariate, and multivariate analysis, I fount that the model can accurately catch fraudulent cases when we do combination of features.
The individual correlations with isFraud were all low, combinations of these features showed much stronger fraud signals than any individual feature alone. 
These are the features that helped guide me for next step to preprocessing the data:
- type:
    - were able to narrow the focus to CASH_OUT and TRANSFER, eliminating the remaining three transaction types from fraud consideration.
- amount: 
...
Here are some of the features I did not find very useful.
- step:
...
The model's precision is 96%, meaning that when it flags a transaction as fraud, it is right almost every time, very few false alarms. However, the model misses 423 actual fraud cases, which is 25.75% of all real fraud in the test set, a low recall. To improve this, future work could apply SMOTE to generate more synthetic fraud examples for training, or increase class weights to make the model pay more attention to fraud cases. Both would help catch more fraud, though some precision would likely be lost in the process.
...
In short, the F1 score tells us how the model performs at the 50% cutoff, while the AUC tells us how good the model is at ranking risk overall. The adjustment I made on the threshold also confirms that tuning it is a quick and effective way to optimize the precision-recall tradeoff for imbalanced datasets.
